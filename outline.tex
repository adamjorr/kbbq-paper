...
\begin{outline}
	...
	\begin{outline}
		...
		\begin{align}
		\hat{Q}_{rg} &= argmax_{Q_{rg}} P(Q_{rg}|Errors_{rg}) \\
		&= argmax_{Q_{rg}} P(\mathcal{B}(Errors_{rg} | Observations_{rg}, Q_{rg}) * P(\mathcal{N}(Q_{rg} | \bar{Q}))
		\end{align}
...
\begin{outline}
	\item \texttt{kbbq} requires as input a set of reads in FASTQ \parencite{cock_sanger_2010} or BAM \parencite{li_sequence_2009} format. If the input data is FASTQ formatted and consists of multiple read groups, the read group each read belongs to should be annotated in the name of each read. Alternatively, if the user has an original data file and a data file that has already been corrected using an error correction program, the user may supply the corrected file to \texttt{kbbq} to obtain errors from that correction rather than performing the error correction algorithm included in \texttt{kbbq}. The program's only required parameter is the approximate length of the genome the reads were sequenced from, and this information can be taken from the BAM header if it is present.

	\item Optionally, the approximate sequencing depth may also be provided. If it is not, it will be estimated by finding the length of the sequenced data divided by the provided genome length; $Depth = \frac{Sequence\:Length}{Genome\:Length}$.
	\item An $\alpha$ parameter may also be provided; this is the same $\alpha$ parameter that Lighter uses, and is the fraction of reads sampled from the input data \parencite{song_lighter_2014}. If not provided, the recommended value of $\alpha = \frac{7}{\operatorname{Depth}}$ is used.
	\item A $k$ parameter may also be provided, which changes the k-mer size used for the error detection algorithm. The maximum value of 32 is recommended.
	\item The genome length parameter, in addition to being used to estimate sequencing depth, is also used to estimate the number of k-mers that will be sampled. This is used to parameterize the bloom filter that stores the sampled and trusted k-mers. This parameterization is different than that used in Lighter; there, the number of sampled k-mers is estimated to be $1.5 \times \operatorname{Genome\:length}$. However, the expected number of sampled k-mers $K_{\operatorname{sampled}}$ is bounded by the expected value of the binomial distribution parameterized by $\operatorname{Genome\:length} \times \operatorname{Depth}$ and $\alpha$. Assuming \textit{every} k-mer is unique, the number of possible k-mers in the dataset $K_{\operatorname{total}}$ is at most $(\operatorname{Read\:length} - k + 1) \times \operatorname{Number\:of\:reads}$, since
	\begin{align}
	K_{total} &= (\operatorname{Read\:length} - k + 1) \times \operatorname{Number\:of\:reads} \\
	&< \operatorname{Read\:length} \times \operatorname{Number\:of\:reads} \\
	&< \operatorname{Read\:length} \times \frac{\operatorname{Depth} \times \operatorname{Genome\:length}}{\operatorname{Read\:length}} \\
	&< \operatorname{Depth} \times \operatorname{Genome\:length}
	\end{align}

	\item So the expected number of sampled k-mers is

	\begin{equation}
	E[K_{\operatorname{sampled}}] = E[\mathcal{B}(x; \alpha, \operatorname{Depth} \times \operatorname{Genome\:length})] =
		\alpha \times \operatorname{Depth} \times \operatorname{Genome\:length}
	\end{equation}

	\item Notably, if $\alpha$ is the recommended value of $\frac{7}{\operatorname{Depth}}$, the expected number of sampled k-mers is less than $7 \times \operatorname{Genome\:length}$, a bound greater than 4 times as large as the estimate used by Lighter. For the data analyzed here, this provides a much better estimate of the true number of sampled k-mers. Ultimately, the larger estimate of elements inserted into the bloom filter causes an increase in size of the bloom filter but a smaller false positive rate.
...
\begin{outline}
	...
	\begin{itemize}
		...
		\item DNA from CHM1 and CHM13 human hydatidiform mole cell lines was mixed to generate a synthetic diploid dataset for benchmarking \parencite{li_synthetic-diploid_2018}.
